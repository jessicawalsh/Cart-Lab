...
int main( int argc, char *argv[] ) {
  ...
  while ((ch = getopt(argc, argv, CART_ARGUMENTS)) != -1) {
  ...
  CartSimulatorLLevel= registerLogLevel("CART_SIMULATOR", 0);    // Simulator log level
  if ( verbose ) {
...
void free_filename_memory(CartSimulationTable ftable[CART_SIM_MAX_OPEN_FILES]) {
  int i;
  ...
  while (!feof(fhandle)) {
    ...
    if (fgets(line, 1024, fhandle) != NULL) {
      ...
      if (strncmp(command, "WRITEAT", 7) == 0) {
        ...
        rbuf = malloc(len);
        if (cart_read(ftable[idx].fhandle, rbuf, len) != off) {
          // Failed, error out
...
int validate_file(char *fname, int16_t mfh) {
  ...
  // Local variables
  char filename[256], bkfile[256], *filbuf = NULL, *membuf = NULL;
  struct stat stats;
  int idx, fh;
  ...
  if ( ((filbuf = malloc(stats.st_size)) == NULL) || ((membuf = calloc(stats.st_size, 1)) == NULL) ) {
    if (filbuf)
      free(filbuf);
    if (membuf)
      free(membuf);
  ...
  for (idx=0; idx<stats.st_size; idx++) {
    if (membuf[idx] != filbuf[idx]) {
      logMessage(LOG_ERROR_LEVEL, "Validation of [%s] failed at offset %d (mem 0x%02x "
          "!= fil 0x%02x/'%c')", fname, idx, membuf[idx], filbuf[idx], filbuf[idx]);
      free(filbuf);
      free(membuf);
      return(-1);
    }
